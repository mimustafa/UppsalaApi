...
namespace UppsalaApi.Services
{
    ...
    {
        private readonly CampusOptions _campusOptions;
        ...
        public TimeSpan GetMinimumStay()
            => TimeSpan.FromHours(_campusOptions.MinimumStayHours);

        public DateTimeOffset FurthestPossibleBooking(DateTimeOffset now)
            => AlignStartTime(now) + TimeSpan.FromDays(_campusOptions.MaxAdvanceBookingDays);

        public DateTimeOffset AlignStartTime(DateTimeOffset original)
        {
            var dateInServerOffset = original.ToOffset(TimeSpan.FromHours(_campusOptions.UtcOffsetHours));
        ...
        public IEnumerable<BookingRange> GetAllSlots(DateTimeOffset start, DateTimeOffset? end = null)
        {
            ...
            while (true)
            {
                ...
                var newEnd = newStart.Add(TimeSpan.FromHours(_campusOptions.MinimumStayHours));
